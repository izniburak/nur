...
class Uri
{
    ...
    */
    ...
    {
        self::$url = http::server('HTTP_HOST') . '/' . BASE_FOLDER . '/';
    ...
    */
    public static function route($name, $params = null)
    {
        ...
        if($found)
        {
            ...
            $data = str_replace(BASE_FOLDER, '', self::$url) . '/' . $newUrl;
            return self::$http . self::replace($data);
        }
        else
    ...
    */
    public static function segment($num = null)
    {
        ...
        if (!is_null($num))
        {
            $uri = self::replace( str_replace(BASE_FOLDER, '', http::server('REQUEST_URI')) );
            $uriA = explode('/', $uri);
